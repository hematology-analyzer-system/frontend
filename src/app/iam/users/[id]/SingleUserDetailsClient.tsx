...
interface SingleUserDetailsClientProps {
  userId: number;
}

export default function SingleUserDetailsClient({ userId }: SingleUserDetailsClientProps) {
  ...
  const handleUpdateUserStatus = async (isLock: boolean) => {
    ...
    try {
      ...
      const res = await fetch(`https://fhard.khoa.email/api/iam/users/${userId}/${status}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
