...
export default async function SingleUserDetailsPage({ params }: UserDetailsProps) {
  ...
  const handleSaveUserDetails = async (updatedUser: UserResponseDTO) => {
    ...
    try {
      ...
      const res = await fetch(`https://fhard.khoa.email/api/iam/users/${resolvedParams.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleUpdateUserStatus = async (isLock: boolean) => {
    ...
    try {
      // Corrected payload key and URL logic
      const status = isLock ? 'unlock' : 'lock';
      const res = await fetch(`https://fhard.khoa.email/api/iam/users/${resolvedParams.id}/${status}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
